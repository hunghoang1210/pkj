...
public class BanRepository {
    ...
    public void insert(Ban model) {
        String sql = "INSERT INTO BAN (MaBan,TenBan,MaKV_FK,SoNguoi,TinhTrang) VALUES (?,?,?,?,?)";
        DBContext.executeUpdate(sql, model.getMaBan(), model.getTenBan(), model.getMaKV(),model.getSoNguoi(),model.getTinhTrang());
    }

    public void update(Ban model) {
        String sql = "UPDATE BAN SET TenBan=?,MaKV_FK=?,SoNguoi=?,TinhTrang=? WHERE MaBan=?";
        DBContext.executeUpdate(sql, model.getMaBan(),model.getTenBan(), model.getMaKV(),model.getSoNguoi(),model.getTinhTrang());
    }
    ...
    private Ban readFromResultSet(ResultSet rs) throws SQLException {
        ...
        model.setSoNguoi(rs.getString("SoNguoi"));
